...
Fue probablemente Eratóstenes quien diseñara la esfera armilar que consiste en un astrolabio para mostrar el movimiento aparente de las estrellas alrededor de la tierra.
...
Para que INDI funcione con total perfección se definen tres entidades muy importantes que hacen que encajen todas las piezas correctamente. Estas entidades son: \textit{drivers}, servidor y cliente.
...
\begin{itemize}
  \item \textbf{Maxim DL:} Dispone de una completa integración del observatorio y controla todo el equipo astronómico. Es compatible con ASCOM y el astrónomo puede crear sus propios preajustes. A su vez, se puede supervisar y controlar el observatorio mediante una webcam en la cúpula, interruptores remotos y vigilancia del clima. Es uno de los \textit{software} sin INDI más completo que existe \cite{MaximDL}.
...
\begin{itemize}
  ...
  \item \textbf{Ekos:} \textit{Software} que lleva a cabo tareas de astrofotografía bajo la plataforma Linux. Ekos es también una herramienta de \textit{Software Libre} y forma parte del \textit{software} de Kstars mencionado anteriormente.
